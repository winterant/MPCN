...
class MPCN(nn.Module):
    ...
    def __init__(self, config, word_emb, fusion_mode='concatenate'):
        super(MPCN, self).__init__()
        self.pointer_count = config.pointer_count
        ...
        if fusion_mode == 'concatenate':
            self.fm = FactorizationMachine(in_dim=2 * self.pointer_count * self.word_dim, k=config.fm_hidden)
        elif fusion_mode == 'sum':
            self.fm = FactorizationMachine(in_dim=2 * self.word_dim, k=config.fm_hidden)
            ...
            print('Please correct the parameter "fusion_mode" when defining the model MPCN!')
            print('It Supports "concatenate", "sum" and "linear".')
            exit(0)
